...
def pre_process_data_remove_timestamp(df):
    df_scaled = df.copy()
    df_scaled['Timestamp'] = pd.to_datetime(df_scaled['Timestamp'])
    df_scaled.set_index(['Timestamp'], inplace=True)
    df_scaled=df_scaled.interpolate(method='linear')
    df_scaled = df_scaled.dropna()
    df_scaled = df_scaled.reset_index()
    df_scaled = df_scaled.drop(columns=['hostname', 'kolla_external_fqdn', 'Timestamp'])
    scaler = MinMaxScaler(feature_range=(0, 1), clip=True)
    x_train = pd.DataFrame(scaler.fit_transform(df_scaled), columns=df_scaled.columns,
...
if __name__ == '__main__':
	...
	df = pd.read_csv("../chameleon-anomaly/node/mgmt01.uc.chameleoncloud.org_group_data.csv")
	x_train = pre_process_data_remove_timestamp(df)
	train_loader = DataLoader(x_train.values, batch_size=x_train.shape[0])

	if args.model in ['MERLIN']:
		eval(f'run_{args.model.lower()}(test_loader, labels, args.dataset)')
	model, optimizer, scheduler, epoch, accuracy_list = load_model(args.model, x_train.shape[1])
	...
	if not args.test:
		...
		for e in tqdm(list(range(epoch+1, epoch+num_epochs+1))):
			lossT, lr = backprop(e, model, trainD, trainD, optimizer, scheduler)
	...
	with open('root_cause_result_OmniAnomaly.json', 'w') as f:
